 ...
 *  4. 将 dist 压缩为 releases/ai-web-reading-assistant-v<版本>.zip
 *  5. 同步版本号到 dist/manifest.json、public/manifest.json、package.json
 *  6. 记录发布状态（版本 + 指纹 + 历史 + 本次 commit 基线）到 releases/version.json，
...
import { decideBump, gitHead, resolveBaseline } from './llm-bump.mjs'
...
async function main() {
  ...
  if (!bump) {
    const decided = await decideBump({ stateFile: STATE_FILE })
    bump = decided.bump
  ...
  // 4. 打包 zip（先临时名，成功后再重命名；失败清理且不生成压缩包）
  ...
  // 6. 记录发布状态（含本次 commit 与差异基线，供下次大模型判定）
  const head = gitHead()
