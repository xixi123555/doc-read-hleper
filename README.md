...
| 3.7 UI 规范 | Vue3 + Naive UI + Tailwind CSS，CSS 变量主题，深浅色一键切换，轻量微动效；弹窗仅保留**设置页**（对话统一走悬浮窗） |
...
- **进入插件**：已配置模型（api-key 存在）→ 点击图标默认**直接唤起悬浮对话窗**，不停留在弹窗；未配置模型 → 弹窗**设置页**完成配置
- **唤起悬浮对话窗**：已配置时点击工具栏图标直接唤起；设置页点「返回对话 / 打开悬浮窗」；快捷键 `Alt+Shift+R`（可自定义）；已配置后如需修改设置，在悬浮对话窗点「⚙️」
